...
select title from film f where f.title like '%academy%';-- 10
...
select concat(c.first_name, ' ',c.last_name) as custname, f.title
from film f 
...
group by c.first_name;-- 17

select concat(act.first_name,' ',act.last_name)as name, A.Num
from actor act
join (select a.actor_id id, count(*) as Num
from actor a
join film_actor f on a.actor_id=f.actor_id
group by a.actor_id
order by count(*) desc
limit 5) as A on A.id=act.actor_id;-- 18

select s.store_id, sum(f.rental_rate) as income from store s
join customer c on c.store_id=s.store_id
join inventory i on i.inventory_id=s.store_id
join film f on f.film_id=i.film_id
join rental r on r.inventory_id=i.inventory_id
group by s.store_id;-- 19

select DATE_FORMAT(rental_date, '%Y-%m') as month,count(*) FROM rental
group by DATE_FORMAT(rental_date, '%Y-%m');-- 20
